...
source("./Generate_forecasts/tg_lm_scale/forecast_model.R")
...
for (th in config$models$tg_lm_scale$model_themes) {
  source_modes_th <- config$source_modes[[th]]
  rerun_forecasts(
    model_id       = config$models$tg_lm_scale$model_id,
    forecast_model = forecast_model,
    model_themes   = th,
    noaa           = TRUE,
    all_sites      = FALSE,
    source_modes   = source_modes_th,
    local_dir      = file.path(config$local_forecasts_dir, "LM_scale")
  )
}
